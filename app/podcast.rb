...
class Podcast
  ...
  def _populate_rss_channel channel
    ...
    channel.title @title or program_property('title')
    channel.link @program_json['link'][1]['$text'] if @program_json['link'] and @program_json['link'][1]
    channel.description program_property('teaser').gsub(%r{</?[^>]+?>}, '')
    channel.language 'en-us'
    channel.copyright @copyright

    # iTunes-specific metadata
    channel.itunes :image, :href => @image_url # NEW! Add podcast image
    channel.itunes :category, :text => 'News & Politics'
    channel.itunes :subtitle, program_property('teaser').gsub(%r{</?[^>]+?>}, '')
    channel.itunes :author, @author
    channel.itunes :summary,  program_property('teaser').gsub(%r{</?[^>]+?>}, '')
  ...
  def _populate_rss_story channel, story_json
    ...
    begin
      image_url = story_json['image'][0]['enlargement']['src'] + '?s=6'
    rescue
      image_url = 'http://dudesbros.com/custom_atc_logo.png'
    end
    ...
    channel.item do |story|
      ...
      # NEW! Code for embedding image in ID3 Tag (requires 'taglib' 'open-uri' and 'uri')
      #  open(audio_url) do |input|
      #    audio_uri = URI.parse(audio_url)
      #    audio_filename = File.basename(audio_uri.path, ".mp4")
      #    # Copy audio locally
      #    local_audio_file = File.new("app/public/audio/#{audio_filename}.m4a", "w+")
      #    File.open(local_audio_file, "wb") do |output|
      #      while buff = input.read(4096)
      #        output.write(buff)
      #      end
      #    end
      #    # Embed local image as cover art
      #    TagLib::MP4::File.open("app/public/audio/#{audio_filename}.m4a") do |mp4|
      ...
      #      mp4.save
      #    end
      #    audio_url = "https://nprpodcasts.herokuapp.com/audio/#{audio_filename}.m4a"
      #  end
